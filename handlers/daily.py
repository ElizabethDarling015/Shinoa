...
async def cb_category_daily(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await call.message.answer(text, parse_mode="HTML", reply_markup=priority_keyboard())
...
async def daily_priority(message: Message, state: FSMContext):
    ...
    if not priority:
        await message.answer(
            ...
            reply_markup=priority_keyboard()  # <-- ИСПРАВЛЕНО: добавлена клавиатура
        )
        return
...
def get_morning_priority_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="⬅️ Отмена", callback_data="cancel_morning"),  # <-- ИСПРАВЛЕНО: уникальный callback
        ]
    ])
...
def get_morning_time_keyboard() -> InlineKeyboardMarkup:
    ...
    bottom_rows = _make_nav_keyboard()
    return InlineKeyboardMarkup(inline_keyboard=[
...
async def morning_text(message: Message, state: FSMContext):
    ...
    if bot_msg_id:
        try:
            ...
            new_msg = await message.answer(question_text, parse_mode="HTML", reply_markup=get_morning_time_keyboard())
            await state.update_data(bot_msg_id=new_msg.message_id)  # <-- ИСПРАВЛЕНО: обновление ID
    else:
        new_msg = await message.answer(question_text, parse_mode="HTML", reply_markup=get_morning_time_keyboard())
        await state.update_data(bot_msg_id=new_msg.message_id)  # <-- ИСПРАВЛЕНО: сохранение ID
...
async def morning_time(message: Message, state: FSMContext):
    ...
    if bot_msg_id:
        try:
            ...
            logger.warning(f"Не удалось отредактировать сообщение приоритета: {e}")
...
async def cb_morning_time_standard(call: CallbackQuery, state: FSMContext):
    ...
    if bot_msg_id and call.message:
        try:
        ...
        if call.message:
            ...
            await call.answer("Сообщение удалено, начните заново через /morning", show_alert=True)
            await state.clear()


@router.callback_query(F.data == "cancel_morning")  # <-- ИСПРАВЛЕНО: новый хендлер
